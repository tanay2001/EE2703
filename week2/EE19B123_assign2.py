...
def cleaner(lines, flag, start, end, ac):
    ...
    for row in cir:
        ...
        if tokens[0][0] in ['R','L','C']:
            ...
            print("sorry , controlled sources isn't supported as of now")
            exit()
        else:
    ...
    try:
        ...
        try:
            ...
            with open(sys.argv[1]) as f:
                ...
                dummy = ' '.join(comment_remover(lines))
                assert START in dummy, 'File missing .circuit'
                assert END in dummy, 'File missing .end'
                assert dummy.index(START) < dummy.index(END), 'follow convention of .circuit --- .end'
                assert dummy.count('RL') ==1 , 'Please use 1 load Resistor ie: name one resistor as RL '
                ...
                for l in lines:
                    ...
                    if AC==tokens[0]:
                        ...
                        ac_flag =True
            if ac_flag:
                assert dummy.index(START) < dummy.index(END)< dummy.index(AC), 'follow convention of .circuit --- .end .ac'
            f.close()
            ...
            i =0
            for n in n2id:
                print("Voltage at {} is {} + j{}".format(n,x[i].real[0].astype(np.float16),\
                    x[i].imag[0].astype(np.float16)))
                i +=1
            for m in s2id:
                print("Current through {} is {} + j{}".format(m,x[i].real[0].astype(np.float16),\
                    x[i].imag[0].astype(np.float16)))
                i+=1
            
            ##TODO print current/
            n1 =n2id[tokenObj['RL'].node1]
            n2 = n2id[tokenObj['RL'].node2]
            current = (x[n1] - x[n2])/tokenObj['RL'].value
            voltage = x[n1]- x[n2]
